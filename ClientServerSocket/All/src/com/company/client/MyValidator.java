    ...
    public boolean checkLessThanAuthorCommand(LinkedHashSet<LabWork> collection) {
        ...
        try {
            ...
            if (author != null) {
                ...
                stringResultOfValidation = CommandInformer.PS1
    ...
    public boolean checkFilterByDifficultyCommand(LinkedHashSet<LabWork> collection) {
        ...
        try {
            ...
            if (wasAtLeastOneThisTypeOfElementInCollection) {
                stringResultOfValidation = CommandInformer.PS1 + "Выведены все лаб. работы типа " + App.USER_COMMAND[1] + ".";
            } else {
                stringResultOfValidation = CommandInformer.PS1 + "В коллекции не нашлось ни одного элемента с заданным типом";
            }
            return true;
        } catch (NullPointerException | IllegalArgumentException exception) {
            return false;
