...
public class FileManager {
        ...
        try {
            ...
            if (file.exists()) {
                if (file.canRead()) {
                    ...
                    System.out.println(CommandManager.PS1 + "У вас нет доступа к файлу.");
                }
                System.out.println(CommandManager.PS1 + "Введите команду 'help' для начала работы.");
            } else {
            ...
            System.out.println(CommandManager.PS1 + "Файл-коллекция по указанному пути не существует.\n" +
    ...
    private void readJsonFile() throws FileNotFoundException {
        ...
        try {
            ...
            if (incorrectLinesNumbersInInputFileCollection.size() == 0) {
                System.out.println(CommandManager.PS1 + "Файл успешно загружен.");
            } else {
                throw new IllegalArgumentException();
            }
        } catch (NullPointerException exception) {
    ...
    public void save(LinkedHashSet<LabWork> collection) {
        try {
            ...
            br.close();
            System.out.println(CommandManager.PS1 + "Коллекция успешно сохранена в файл.");
        } catch (FileNotFoundException exception) {
            System.out.println(CommandManager.PS1 + "Файл-коллекция не найден.");
        } catch (IOException e) {
            e.printStackTrace();
        }
